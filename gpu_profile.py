...
gpu_profile_fn = fstring('{datetime.datetime.now():%d-%b-%y-%H:%M:%S}-gpu_mem_prof.txt')
        ...
        try:
            ...
            if lineno is not None:
                ...
                with open(gpu_profile_fn, 'a+') as f:
                    f.write(fstring("{where_str:<50}")+
                            fstring(":{meminfo.used/1024**2:<7.1f}Mb ")+
                            fstring("{line.rstrip()}\n"))
                    ...
                    if print_tensor_sizes is True:
                        ...
                        for t, s, loc in new_tensor_sizes - last_tensor_sizes:
                            f.write(fstring('+ {loc:<50} {str(s):<20} {str(t):<10}\n'))
